(function () {
  ...
  const STORAGE_KEY = "pfh_multi_subject_state_v1";
  const LEGACY_STORAGE_KEY = "pfh_math_sec_state_v3";
  ...
  const subjectApi = window.PFH_SUBJECT;
  const historySubjectApi = window.PFH_HISTORY_SUBJECT;
  ...
  function getSubject() {
    return subjectApi && subjectApi.getSubject
      ? subjectApi.getSubject(state.category)
      : { id: "数学", name: "数学", logo: "数", tagline: "填几句课堂观察，自动生成发给家长的数学课反馈", placeholders: {}, examples: {} };
  }

  function listSubjects() {
    return subjectApi && subjectApi.listSubjects ? subjectApi.listSubjects() : [getSubject()];
  }

  function trackUsageEvent(event, fields = {}) {
    if (!window.PFH_USAGE || typeof window.PFH_USAGE.track !== "function") return "";
    return window.PFH_USAGE.track(event, {
  ...
  function createUsageEventId(prefix) {
    if (window.PFH_USAGE && typeof window.PFH_USAGE.createEventId === "function") {
      return window.PFH_USAGE.createEventId(prefix);
    }
    return `${prefix || "e"}_${Date.now().toString(36)}`;
  }

  function getUsageStudentCount() {
    if (state.lessonMode === "one") {
      return getCleanValue("#ooName") ? 1 : 0;
    }
    return state.students.filter((s) => cleanDraftText(s.name)).length;
  }

  function getAiStyleUsageFlags() {
    const cfg = templateApi.normalizeConfig(state.templateConfig);
    const aiConfig = cfg.aiConfig || {};
    return {
      hasAiStyleExamples: !!String(aiConfig.examples || "").trim(),
      hasAiStyleAvoid: !!String(aiConfig.avoid || "").trim(),
    };
  }

  function isTemplateConfigCustomized() {
    try {
      return JSON.stringify(templateApi.normalizeConfig(state.templateConfig))
        !== JSON.stringify(templateApi.normalizeConfig(templateApi.DEFAULT_TEMPLATE_CONFIG));
    } catch (e) {
      return false;
    }
  }

  function getFallbackReason(err) {
    const code = err && typeof err.code === "string" ? err.code : "";
    const status = err && Number.isFinite(err.status) ? err.status : 0;
    if (code === "RATE_LIMITED" || status === 429) return "rate_limited";
    if (code === "CIRCUIT_OPEN" || status === 503) return "circuit_open";
    if (code === "TIMEOUT" || code === "UPSTREAM_TIMEOUT" || status === 504) return "timeout";
    if (code === "UPSTREAM_ERROR" || status >= 500) return "upstream_error";
    if (code === "NETWORK_ERROR") return "fetch_failed";
    return "unknown";
  }

  function cleanDraftText(value) {
    return subjectApi && subjectApi.cleanDraftText
      ? subjectApi.cleanDraftText(value)
      : String(value || "").trim().replace(/^(?:undefined\s*)+/i, "").trim();
  }

  function isDirtyCachedText(value) {
    return subjectApi && subjectApi.isDirtyCachedText
      ? subjectApi.isDirtyCachedText(value)
      : /^\s*undefined/i.test(String(value || ""));
  }

  function cleanAndTrack(value) {
    if (isDirtyCachedText(value)) restoredHadDirtyCache = true;
    return cleanDraftText(value);
  }

  function setCleanValue(sel, value) {
    const el = $(sel);
    if (el) el.value = cleanAndTrack(value);
  }

  function getCleanValue(sel) {
    const el = $(sel);
    if (!el) return "";
    const clean = cleanDraftText(el.value);
    if (clean !== el.value.trim()) el.value = clean;
    return clean;
  }
  ...
  function getModeMeta(mode) {
    ...
    if (mode === "one") {
      return {
        tagline: `记录孩子本节${subject.name}掌握情况、主要卡点和下次辅导计划`,
        btn: "生成一对一反馈",
      };
    }
    return {
      tagline: subject.tagline || "填几句课堂观察，自动生成发给家长的课后反馈",
      btn: "一键生成全班反馈",
    };
  }

  const ONE_FIELDS = ["#ooName", "#ooGrade", "#ooMastered", "#ooWeakness", "#ooPractice", "#ooParent", "#ooNextPlan"];
  const FORM_FIELDS = ["#lessonTime", "#lessonNo", "#topic", "#homework", "#classNote", "#classContent"].concat(ONE_FIELDS);

  // 「我的格式」预览用的示例数据
  function buildPreviewSample() {
    const subject = getSubject();
    const sample = subject.examples && subject.examples.class ? subject.examples.class : {};
    const student = sample.students && sample.students[0] ? sample.students[0] : { name: "张晨", keywords: "课堂表现认真" };
    return {
      lessonTime: sample.lessonTime || "2026年6月15日",
      lessonNo: sample.lessonNo || "第8次课",
      studentName: student.name || "张晨",
      knowledgePoints: sample.topic || "本节课核心内容",
      homework: sample.homework || "按老师课堂要求完成对应练习",
      teacherComment:
        `${student.name || "孩子"}今天${student.keywords || "课堂表现认真"}，课后按课堂要求巩固即可。`,
    };
  }

  const VIEW_META = {
    write: { title: "写反馈", tagline: "" },
  ...
  let restoredHadDirtyCache = false;
  ...
  function showView(name) {
    ...
    $("#viewTagline").textContent = meta.tagline || getModeMeta(state.lessonMode).tagline;
  ...
  function genLabel() {
    if (state.lessonMode === "class") {
      ...
      return count ? `大班 ${count} 名，生成反馈` : getModeMeta("class").btn;
    }
    return getModeMeta(state.lessonMode).btn;
  }
  ...
  function renderSubjectTabs() {
    ...
    listSubjects().forEach((subject) => {
      ...
      btn.type = "button";
      btn.className = "subject-tab" + (subject.id === state.category ? " active" : "");
  ...
  function bindSubjectSwitch() {
    ...
    wrap.addEventListener("click", (e) => {
      ...
      if (!btn) return;
      const hasDraft = hasAnyDraftContent();
      const shouldUseExample = isExampleOrDirtyDraft();
      state.category = btn.dataset.subject || "数学";
      ...
      if (shouldUseExample) {
        ...
        hideSubjectDraftNotice();
        syncStudents();
        restoredHadDirtyCache = false;
      }
      saveState();
      if (!shouldUseExample && hasDraft) {
        showSubjectDraftNotice();
        toast(`已切换到${state.category}，当前草稿已保留`);
      } else {
        toast(`已切换到${state.category}`);
      }
    });
  }
  ...
  function applySubject() {
    const subject = getSubject();
    state.category = subject.id || "数学";
    const placeholders = subject.placeholders || {};
    document.title = `${subject.title || "课后反馈助手"} · ${subject.name}家长反馈`;
    $("#subjectLogo").textContent = subject.logo || subject.name.slice(0, 1);
    renderSubjectTabs();
    applyMode(state.lessonMode);

    $("#topicLabel").textContent = subject.topicLabel || "今天的课程主题";
    $("#topicHint").textContent = `（${subject.topicHint || "教了什么"}）`;
    $("#topic").placeholder = placeholders.topic || "";
    $("#homework").placeholder = placeholders.homework || "";
    $("#classNote").placeholder = placeholders.classNote || "";
    $("#classContentLabel").textContent = subject.contentLabel || "课堂练习 / 讲解内容";
    $("#classContent").placeholder = placeholders.classContent || "";
    $("#batchText").placeholder = placeholders.batch || "";
    const studentHint = $("#studentInputHint");
    if (studentHint) studentHint.textContent = "先写亮点，再写一个待加强点；可用逗号、分号或顿号分隔。";
    $("#ooMastered").placeholder = placeholders.mastered || "";
    $("#ooWeakness").placeholder = placeholders.weakness || "";
    $("#ooPractice").placeholder = placeholders.practice || "";
    $("#ooNextPlan").placeholder = placeholders.nextPlan || "";
    const formatNote = $("#formatScopeNote");
    if (formatNote) {
      formatNote.textContent = `当前格式适用于全部学科；本次生成会按「${subject.name}」的课堂表达和示例生成内容。`;
    }
    updateStudentKeywordPlaceholders();
    updatePreview();
    updateMockNote(window.PFH_LLM && window.PFH_LLM.isEnabled());
    renderStudentAssets();
    renderHistory();
    updateReferenceNotice();
  }

  function updateStudentKeywordPlaceholders() {
    const placeholder = (getSubject().placeholders || {}).studentKeywords || "例如：课堂亮点；待加强点";
    document.querySelectorAll(".student-row .kw").forEach((input) => {
  ...
  function makeStudentRow(name = "", keywords = "") {
    const placeholders = getSubject().placeholders || {};
    const row = document.createElement("div");
    row.className = "student-row";
    row.innerHTML = `
      <input type="text" class="name" placeholder="姓名" value="${escapeAttr(name)}" />
      <input type="text" class="kw" placeholder="${escapeAttr(placeholders.studentKeywords || "例如：课堂亮点；待加强点")}" value="${escapeAttr(keywords)}" />
  ...
  function syncStudents() {
    ...
    state.students = rows.map((r) => {
      ...
      const keywordInput = r.querySelector(".kw");
      const name = cleanDraftText(nameInput.value);
      const keywords = cleanDraftText(keywordInput.value);
      if (name !== nameInput.value.trim()) nameInput.value = name;
      if (keywords !== keywordInput.value.trim()) keywordInput.value = keywords;
  ...
  function updatePreview() {
    const text = templateApi.renderFromConfig(state.templateConfig, buildPreviewSample());
    $("#tplPreview").textContent = text;
  }

  function renderAiStyle() {
    const cfg = templateApi.normalizeConfig(state.templateConfig);
    state.templateConfig = cfg;
    const examples = $("#aiStyleExamples");
    const avoid = $("#aiStyleAvoid");
    if (examples) examples.value = cfg.aiConfig.examples;
    if (avoid) avoid.value = cfg.aiConfig.avoid;
    updateAiStyleNote();
    updateAiStyleStatus(false);
  }

  function updateAiStyleNote() {
    const note = $("#aiStyleAiNote");
    if (!note) return;
  ...
  function updateAiStyleStatus(saved) {
    const status = $("#aiStyleStatus");
    if (!status) return;
    status.textContent = saved ? "已保存" : "自动保存";
  }

  function saveAiStyle() {
    const cfg = templateApi.normalizeConfig(state.templateConfig);
    cfg.aiConfig = templateApi.normalizeAiConfig({
      examples: $("#aiStyleExamples") ? $("#aiStyleExamples").value : "",
      avoid: $("#aiStyleAvoid") ? $("#aiStyleAvoid").value : "",
    });
    state.templateConfig = cfg;
    saveState();
    updateAiStyleStatus(true);
  }

  function resetTemplate() {
    const current = templateApi.normalizeConfig(state.templateConfig);
    state.templateConfig = {
      ...deepClone(templateApi.DEFAULT_TEMPLATE_CONFIG),
  ...
  function hasAssetsForCurrentSubject() {
    if (!assetsApi) return false;
    return assetsApi.getRecentGroups(studentAssets, 1, { subject: state.category }).length > 0
      || assetsApi.getRecentStudents(studentAssets, 1, { subject: state.category }).length > 0;
  }
  ...
  function updateAssetSummary() {
    ...
    const groups = assetsApi.getRecentGroups(studentAssets, 1, { subject: state.category });
    const students = assetsApi.getRecentStudents(studentAssets, 8, { subject: state.category });
    if (groups.length) {
      el.textContent = groups[0].name;
    } else if (students.length) {
      el.textContent = `${students.length} 位最近学生`;
    } else {
      el.textContent = "添加学生并生成后自动保存";
    }
  }
  ...
  function restoreFoldDefaults() {
    const savedAssetFold = localStorage.getItem(ASSET_FOLD_KEY);
    const savedHistoryFold = localStorage.getItem(HISTORY_FOLD_KEY);
    assetCollapsed = savedAssetFold === null ? hasAssetsForCurrentSubject() : savedAssetFold !== "0";
    historyCollapsed = savedHistoryFold === null ? getCurrentSubjectHistory().length > 0 : savedHistoryFold !== "0";
  }
  ...
  function updateHistorySummary() {
    ...
    if (!el) return;
    const history = getCurrentSubjectHistory();
    if (!history.length) {
      el.textContent = "生成后自动保存";
      return;
    }
  ...
  function renderStudentChips() {
    ...
    if (!students.length) {
      ...
      wrap.textContent = `添加学生并生成${state.category}反馈后，会自动保存到这里；也可保存当前名单为班级。`;
      return;
    }
  ...
  function renderGroupChips() {
    ...
    if (!wrap) return;
    const groups = assetsApi.getRecentGroups(studentAssets, 3, { subject: state.category });
    if (!groups.length) {
      wrap.className = "chip-list empty-note";
      wrap.textContent = `还没有${state.category}常用班级。填好名单后点「保存当前名单」，下次可一键带入。`;
      return;
    }
    wrap.className = "chip-list";
    wrap.innerHTML = "";
    groups.forEach((group) => {
      ...
      btn.addEventListener("click", () => loadGroupStudents(group));
      wrap.appendChild(btn);
    });
  }
  ...
  function loadGroupStudents(group) {
    ...
    const groupStudents = assetsApi.getGroupStudents(studentAssets, groupId, { subject: state.category });
    const students = assetsApi.buildGroupStudentReplacement(groupStudents);
    if (!students.length) {
    ...
    students.forEach((student) => {
      studentList.appendChild(makeStudentRow(student.name, student.keywords || ""));
    });
    restoredHadDirtyCache = false;
    syncStudents();
    renderStudentAssets();
    toast(`已载入 ${group && group.name ? `${group.name} · ` : ""}${students.length} 名学员`);
  }
  ...
  function saveCurrentGroup() {
    ...
    const name = prompt("给这组学生起个名字", state.topic ? `${state.category}·${state.topic}班` : `${state.category}班级`);
    if (!name || !name.trim()) return;
    studentAssets = assetsApi.saveGroupFromStudents(studentAssets, name.trim(), valid, { subject: state.category });
    saveStudentAssets();
    renderStudentAssets();
    toast("已保存为常用班级");
  }
  ...
  function trackCurrentStudentsUsage(students) {
    if (!assetsApi) return;
    studentAssets = assetsApi.trackStudentUsage(studentAssets, students || [], { subject: state.category });
    saveStudentAssets();
    renderStudentAssets();
  }
  ...
  function resetFormValues() {
    FORM_FIELDS.forEach((sel) => { $(sel).value = ""; });
    studentList.innerHTML = "";
  }
  ...
  function fillSubjectExample() {
    ...
    const classExample = examples.class || {};
    const oneExample = examples.one || {};
    $("#lessonTime").value = classExample.lessonTime || "";
    $("#lessonNo").value = classExample.lessonNo || "";
    $("#topic").value = classExample.topic || "";
    $("#homework").value = classExample.homework || "";
    $("#classNote").value = classExample.classNote || "";
    (classExample.students || []).forEach((student) => addStudent(student.name, student.keywords));

    $("#classContent").value = oneExample.classContent || "";
    $("#ooName").value = oneExample.name || "";
    $("#ooGrade").value = oneExample.grade || "";
    $("#ooMastered").value = oneExample.mastered || "";
    $("#ooWeakness").value = oneExample.weakness || "";
    $("#ooPractice").value = oneExample.practice || "";
    $("#ooParent").value = oneExample.parent || "";
    $("#ooNextPlan").value = oneExample.nextPlan || "";
  }

  function useCurrentSubjectExample() {
    resetFormValues();
    fillSubjectExample();
    $("#sampleCard").classList.remove("hidden");
    hideSubjectDraftNotice();
    ...
    syncStudents();
    saveState();
    toast(`已换成${state.category}示例课`);
  }

  function markFormalDraft() {
    const sampleCard = $("#sampleCard");
    if (sampleCard && !sampleCard.classList.contains("hidden")) {
      sampleCard.classList.add("hidden");
      toast("已进入正式填写");
    }
  }
  ...
  function showSubjectDraftNotice() {
    const notice = $("#subjectDraftNotice");
    const text = $("#subjectDraftNoticeText");
    if (!notice || !text) return;
    text.textContent = `已切换到${state.category}，当前草稿已保留。`;
    notice.classList.remove("hidden");
  }

  function hideSubjectDraftNotice() {
    const notice = $("#subjectDraftNotice");
    if (notice) notice.classList.add("hidden");
  }

  function isExampleOrDirtyDraft() {
    const sampleCard = $("#sampleCard");
    if (sampleCard && !sampleCard.classList.contains("hidden")) return true;
    if (restoredHadDirtyCache) return true;
    const values = FORM_FIELDS.map((sel) => ($(sel) && $(sel).value) || "");
    const studentValues = state.students.flatMap((student) => [student.name || "", student.keywords || ""]);
    if (values.concat(studentValues).some(isDirtyCachedText)) return true;
    return matchesAnySubjectExample();
  }

  function matchesAnySubjectExample() {
    const topic = getCleanValue("#topic");
    const homework = getCleanValue("#homework");
    const classNote = getCleanValue("#classNote");
    const currentStudents = state.students
      .filter((student) => cleanDraftText(student.name) || cleanDraftText(student.keywords))
      .map((student) => ({
        name: cleanDraftText(student.name),
        keywords: cleanDraftText(student.keywords),
      }));

    return listSubjects().some((subject) => {
      const example = (subject.examples && subject.examples.class) || {};
      if (!example.topic || topic !== example.topic) return false;
      if (homework && homework !== example.homework) return false;
      if (classNote && classNote !== example.classNote) return false;
      const exampleStudents = example.students || [];
      if (!currentStudents.length) return true;
      return currentStudents.every((student, index) => {
        const ex = exampleStudents[index] || {};
        return student.name === ex.name && student.keywords === ex.keywords;
      });
    });
  }

  function hasAnyDraftContent() {
    const values = FORM_FIELDS.map((sel) => ($(sel) && $(sel).value ? cleanDraftText($(sel).value) : ""));
    if (values.some(Boolean)) return true;
    return state.students.some((student) => cleanDraftText(student.name) || cleanDraftText(student.keywords));
  }
  ...
  function updateMockNote(aiOn) {
    ...
    const subject = getSubject();
    note.textContent = aiOn
      ? `已开启 AI 智能生成：${subject.name}点评由 DeepSeek 撰写，仍按你的「我的格式」排版；服务异常会自动回退本地模板。`
      : (subject.mockNote || "当前为本地模板生成模式。打开上方「AI 智能生成」可让大模型写出更自然、更个性化的点评。");
    updateAiStyleNote();
  }
  ...
  async function generate() {
    const sampleCard = $("#sampleCard");
    if (sampleCard && !sampleCard.classList.contains("hidden")) {
      toast("当前是示例课，生成结果仅用于体验；正式使用请先清空或修改字段");
    }
    const clientEventId = createUsageEventId("gen");
    trackUsageEvent("generate_click", {
      clientEventId,
      studentCount: getUsageStudentCount(),
      aiEnabled: !!(window.PFH_LLM && window.PFH_LLM.isEnabled()),
      ...getAiStyleUsageFlags(),
    });
    if (state.lessonMode === "one") return generateOne(clientEventId);
    return generateClass(clientEventId);
  }

  async function generateClass(clientEventId) {
    syncStudents();
    state.lessonTime = getCleanValue("#lessonTime");
    state.lessonNo = getCleanValue("#lessonNo");
    state.topic = getCleanValue("#topic");
    state.homework = getCleanValue("#homework");
    state.classNote = getCleanValue("#classNote");
    ...
    if (aiOn && aiFellBack) {
      trackUsageEvent("ai_fallback", {
        clientEventId,
        reason: fallbackReason,
        studentCount: valid.length,
      });
      toast("AI 暂不可用，已用本地模板生成");
    }
  ...
  async function generateOne(clientEventId) {
    const name = getCleanValue("#ooName");
    if (!name) {
      toast("请先填写学生姓名");
      return;
    }
    ...
    const input = {
      ...
      tone: state.tone,
      clientEventId,
      lessonTime: getCleanValue("#lessonTime"),
      lessonNo: getCleanValue("#lessonNo"),
      topic: getCleanValue("#topic"),
      homework: getCleanValue("#homework"),
      classContent: getCleanValue("#classContent"),
      student: { name, grade: getCleanValue("#ooGrade") },
      mastered: getCleanValue("#ooMastered"),
      weakness: getCleanValue("#ooWeakness"),
      practiceAdvice: getCleanValue("#ooPractice"),
      parentAdvice: getCleanValue("#ooParent"),
      nextPlan: getCleanValue("#ooNextPlan"),
  ...
  function loadHistory() {
    try {
      ...
      return assetsApi ? assetsApi.migrateHistoryEntries(data, { subject: "数学" }) : (Array.isArray(data) ? data : []);
    } catch (e) {
      return [];
    }
  }

  function getCurrentSubjectHistory() {
    return loadHistory().filter((item) => (item.subject || "数学") === state.category);
  }
  ...
  function renderHistory() {
    ...
    if (!visible.length) {
      ...
      empty.className = "empty-note history-empty";
      empty.textContent = `生成一次${state.category}反馈后，会在这里自动保存；之后可恢复填写、参考上次反馈或复制结果。`;
    ...
    visible.slice(0, 3).forEach((item) => {
      ...
      row.innerHTML = `
        ...
        <div class="history-meta">${escapeHtml(item.subject || state.category)} · ${escapeHtml(mode)} · ${item.studentCount || 0} 条${escapeHtml(studentText)} · ${escapeHtml(formatDisplayTime(item.createdAt))}</div>
  ...
  function restoreHistory(item) {
    const snapshot = resolveHistorySnapshot(item);
  ...
  function resolveHistorySnapshot(item) {
    const snapshot = item && item.formSnapshot ? item.formSnapshot : null;
    if (historySubjectApi && typeof historySubjectApi.withSnapshotCategory === "function") {
      return historySubjectApi.withSnapshotCategory(snapshot, item && item.subject, state.category);
    }
    const base = snapshot && typeof snapshot === "object" ? snapshot : {};
    return {
      ...base,
      category: String(base.category || (item && item.subject) || state.category || "数学").trim() || "数学",
    };
  }
  ...
  function copyHistory(item) {
    ...
    copyText(text);
    trackUsageEvent("copy_result", {
      mode: item.lessonMode || state.lessonMode,
      subject: item.subject || state.category,
  ...
  function clearHistory() {
    if (!confirm(`确定清空最近生成的${state.category}记录吗？`)) return;
    const history = loadHistory().filter((item) => (item.subject || "数学") !== state.category);
    if (history.length) {
      localStorage.setItem(HISTORY_KEY, JSON.stringify(history));
    } else {
  ...
  function createFormSnapshot() {
    return {
      ...
      topic: getCleanValue("#topic"),
      homework: getCleanValue("#homework"),
      classNote: getCleanValue("#classNote"),
      classContent: getCleanValue("#classContent"),
      students: deepClone(state.students),
      oneOnOne: {
        name: getCleanValue("#ooName"),
        grade: getCleanValue("#ooGrade"),
        mastered: getCleanValue("#ooMastered"),
        weakness: getCleanValue("#ooWeakness"),
        practice: getCleanValue("#ooPractice"),
        parent: getCleanValue("#ooParent"),
        nextPlan: getCleanValue("#ooNextPlan"),
      },
    };
  }
  ...
  function applySnapshot(snapshot) {
    ...
    setCleanValue("#lessonTime", snapshot.lessonTime);
    setCleanValue("#lessonNo", snapshot.lessonNo);
    setCleanValue("#topic", snapshot.topic);
    setCleanValue("#homework", snapshot.homework);
    setCleanValue("#classNote", snapshot.classNote);
    setCleanValue("#classContent", snapshot.classContent);
    const oo = snapshot.oneOnOne || {};
    setCleanValue("#ooName", oo.name);
    setCleanValue("#ooGrade", oo.grade);
    setCleanValue("#ooMastered", oo.mastered);
    setCleanValue("#ooWeakness", oo.weakness);
    setCleanValue("#ooPractice", oo.practice);
    setCleanValue("#ooParent", oo.parent);
    setCleanValue("#ooNextPlan", oo.nextPlan);
    studentList.innerHTML = "";
    (snapshot.students || []).forEach((st) => addStudent(cleanAndTrack(st.name), cleanAndTrack(st.keywords)));
    applySubject();
    applyMode(state.lessonMode);
  }
  ...
  function loadState() {
    try {
      ...
      state.category = s.category || s.subject || "数学";
      if (s.tone) {
        state.tone = s.tone;
      ...
      setCleanValue("#lessonTime", s.lessonTime);
      setCleanValue("#lessonNo", s.lessonNo);
      setCleanValue("#topic", s.topic);
      setCleanValue("#homework", s.homework);
      setCleanValue("#classNote", s.classNote);
      setCleanValue("#classContent", s.classContent);
      const oo = s.oneOnOne || {};
      setCleanValue("#ooName", oo.name);
      setCleanValue("#ooGrade", oo.grade);
      setCleanValue("#ooMastered", oo.mastered);
      ...
      if (Array.isArray(s.students) && s.students.length) {
        s.students.forEach((st) => addStudent(cleanAndTrack(st.name), cleanAndTrack(st.keywords)));
      }
      return true;
    } catch (e) {}
    return false;
  ...
  function init() {
    ...
    $("#useSubjectExample").addEventListener("click", useCurrentSubjectExample);
    ...
    $("#aiStyleExamples").addEventListener("input", saveAiStyle);
    $("#aiStyleAvoid").addEventListener("input", saveAiStyle);
    FORM_FIELDS.forEach((sel) => {
      $(sel).addEventListener("input", handleDraftInput);
    });

    const restored = loadState();
    applySubject();
    if (restoredHadDirtyCache) saveState();
    if (restored) $("#sampleCard").classList.add("hidden");
    if (!restored) {
      // 预填当前学科示例，方便老师演示时立刻看到效果
      fillSubjectExample();
    }
    if (state.lessonMode === "class" && !studentList.querySelector(".student-row")) {
      addStudent();
    }
    syncStudents();
    renderBuilder();
    // 首次无数据默认展开；有用户折叠偏好时尊重偏好。
    restoreFoldDefaults();
    applyAssetFold();
    renderHistory();
    renderStudentAssets();
    ...
    trackUsageEvent("page_view", {
      hasSavedDraft: restored,
      hasTemplateConfig: isTemplateConfigCustomized(),
    });
  }
