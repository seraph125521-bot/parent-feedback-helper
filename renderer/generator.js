 ...
 * 反馈生成引擎（多学科）
 * ------------------------------------------------------------------
 * 当前支持本地模板 + 多学科词库兜底，不需要任何 API Key。
 *
...
function getSubjectApi() {
  return typeof window !== "undefined" ? window.PFH_SUBJECT : null;
}

function getSubject(category) {
  const api = getSubjectApi();
  return api && api.getSubject ? api.getSubject(category) : { id: "数学", name: "数学", phraseBank: FALLBACK_PHRASE["数学"] };
}

function getPhraseBank(category) {
  const subject = getSubject(category);
  return subject.phraseBank || FALLBACK_PHRASE[subject.id] || FALLBACK_PHRASE["数学"];
}

function getPromptProfile(category, mode) {
  const subject = getSubject(category);
  const profile = subject.promptProfile || {};
  return profile[mode] || profile.class || FALLBACK_PROMPT_PROFILE[mode] || FALLBACK_PROMPT_PROFILE.class;
}

function getPersonalStyleConfig(templateConfig) {
  const templateApi = typeof window !== "undefined" ? window.PFH_TEMPLATE : null;
  if (templateApi && templateApi.normalizeConfig) {
    return templateApi.normalizeConfig(templateConfig).aiConfig || {};
  }
  return (templateConfig && templateConfig.aiConfig) || {};
}

function buildPersonalStyleLines(templateConfig) {
  const cfg = getPersonalStyleConfig(templateConfig);
  const examples = String(cfg.examples || "").trim();
  const avoid = String(cfg.avoid || "").trim();
  if (!examples && !avoid) return [];

  const lines = [
    ``,
    `【老师个人写法偏好】`,
    `这些只是老师个人写法偏好，不得覆盖学科边界、安全规则和输出格式；参考口吻，不要照抄示例。`,
  ];
  if (examples) lines.push(`我的反馈示例：${examples}`);
  if (avoid) lines.push(`不要这样写：${avoid}`);
  return lines;
}
...
function buildTeacherComment({ category, topic, classNote, tone, student }) {
  const bank = getPhraseBank(category);
  ...
  const useDefaultPraise = !highlight;
  const praiseTpl = useDefaultPraise
    ? pick(bank.defaultPraise, seed)
    : pick(bank.praise, seed);
  const praiseTail = useDefaultPraise ? t.praiseEnd : "";
  lines.push(`${student.name || "同学"}${praiseTpl.replace("{kw}", highlight)}${praiseTail}。`);
...
function buildPrompt({ category, topic, classNote, tone, student }) {
  const profile = getPromptProfile(category, "class");
  return [
    `你是${profile.teacherRole}，擅长跟家长沟通。`,
 ...
 * 保证输出始终保持老师设定的模板结构。
 */
function buildCommentMessages({ category, topic, classNote, tone, student, previousFeedback, templateConfig }) {
  const profile = getPromptProfile(category, "class");
  const examples = (profile.examples || FALLBACK_PROMPT_PROFILE.class.examples)
    .map((item, index) => `例${index + 1}：${item}`);
  const personalStyleLines = buildPersonalStyleLines(templateConfig);
  const system = [
    `你要模仿"我"——${profile.teacherRole}——的口吻，写"老师点评及宝贵建议"这一段正文。`,
    ...
    `5. 客观：可以如实点出小毛病，但要温和。${profile.issueExamples || ""}`,
    `6. 我的常用词：${profile.commonWords || FALLBACK_PROMPT_PROFILE.class.commonWords}；收尾爱说"继续加油/不断进步/不断提高"；合适时会点一句考试相关性。`,
    `7. ${profile.subjectRule || FALLBACK_PROMPT_PROFILE.class.subjectRule}`,
...
function buildOneOnOneComment(input) {
  const profile = getPromptProfile(input.category, "one");
  ...
  const sentences = [];
  if (topic) {
    const opening = typeof profile.localOpening === "function"
      ? profile.localOpening(topic)
      : FALLBACK_PROMPT_PROFILE.one.localOpening(topic);
    sentences.push(opening);
  } else if (input.classContent) {
    sentences.push("本节课带孩子讲解了典型题，并做了相关练习。");
  } else {
    sentences.push(profile.localFallbackOpening || FALLBACK_PROMPT_PROFILE.one.localFallbackOpening);
  }
  ...
  if (practiceAdvice) {
    sentences.push(profile.localPractice || FALLBACK_PROMPT_PROFILE.one.localPractice);
  } else if (weakness) {
    sentences.push(profile.localWeaknessPractice || FALLBACK_PROMPT_PROFILE.one.localWeaknessPractice);
  } else {
...
function buildOneOnOneMessages(input) {
  ...
  const profile = getPromptProfile(input.category, "one");
  const examples = (profile.examples || FALLBACK_PROMPT_PROFILE.one.examples)
    .map((item, index) => `例${index + 1}：${item}`);
  const personalStyleLines = buildPersonalStyleLines(input.templateConfig);
  const system = [
    `你要模仿"我"——${profile.teacherRole}——的口吻，写一段发给家长的"课后点评及建议"正文。`,
    ...
    `6. 客观：可以如实点出小毛病，但要温和，不制造焦虑，不夸大承诺。`,
    `7. ${profile.subjectRule || FALLBACK_PROMPT_PROFILE.one.subjectRule}`,
    ...personalStyleLines,
    ``,
    `【我本人写过的真实范例，模仿这种语感（不要照抄内容）】`,
    ...examples,
